...
def compute_field_parameters(db_data: np.ndarray, db_metadata: dict, scalebreak: Optional[float] = None):
    """
...
def correlation_length(lag1: float, lag2: float, dx=10, tol=1e-4, max_lag=1000) -> float:
    """
...
def power_law_acor(config: Dict[str, Any], T_ref: float) -> tuple[np.ndarray, np.ndarray]:
    """
