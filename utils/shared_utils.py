...
def update_field(
    ...
    for ilev in range(n_levels):
        ...
        if ar_order == 2:
            gamma_2 = adjust_lag2_corrcoef2(
                gamma_1, gamma_2)
            phi[ilev] = estimate_ar_params_yw(
                [gamma_1, gamma_2])
        else:
            phi[ilev] = estimate_ar_params_yw(
                [gamma_1])
    ...
    if ar_order == 2:
        ...
        for ilev in range(n_levels):
            adv_lag2 = extrapolation_method(
                lag_2[ilev], oflow, 2, outval=0)[1]
            adv_lag1 = extrapolation_method(
                lag_1[ilev], oflow, 1, outval=0)[0]
...
def qc_params(ens_df, config):
    ...
    """
    var_list = [
        "nonzero_mean_db", "nonzero_stdev_db", "rain_fraction",
        "beta_1", "beta_2"
    ]
    ...
    for iv, var in enumerate(var_list):
        x_list = [
            np.nan if qc_df.at[idx, "param"].get(var) is None
        ...
        qc_dict[var] = np.clip(
            model.fittedvalues, var_lower[iv], var_upper[iv]
        )
...
def blend_parameters(
    ...
    for vtime in blended_df.index:
        ...
        updated = blend_param(rad_param, clean_original,
                              blended_param_names, weight)
...
def fill_param_gaps(ens_df: pd.DataFrame, forecast_times: list[datetime.datetime]) -> pd.DataFrame:
    """
    ...
    for vtime in forecast_times:
        ...
        if original_times:
            ...
            logging.debug(
                f"No valid parameter found near {vtime}, using default")
            def_param = StepsParameters(metadata=metadata)

        filled_map[vtime] = def_param

    records = [{"valid_time": t, "param": p}
               for t, p in sorted(filled_map.items())]
...
def fit_auto_cors(
    ...
    *,
    ...
    try:
        ...
        res = minimize_scalar(_obj, bounds=(lo, hi), method="bounded", options={"xatol": 1e-10})
        lag1 = float(res.x)
    except Exception:
